...
#define PA_VALLOC  _IOWR('P', 0x10, struct pa_req)
#define PA_VFREE   _IOWR('P', 0x11, struct pa_req)
static struct kmem_cache *pa_vcache;
...
static long pa_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
	case PA_ALLOC: {
		void *obj = kmalloc(OBJ_SIZE, GFP_KERNEL_ACCOUNT);
		if (!obj) { 
			ret = -ENOMEM; 
			break; 
		}
	...
	case PA_POKE:
		...
		break;
	case PA_VALLOC: {
		void *obj = kmem_cache_alloc(pa_vcache, GFP_KERNEL);
		if (!obj) { 
			ret = -ENOMEM; 
			break; 
		}
	...
	case PA_VFREE:
		...
		kmem_cache_free(pa_vcache, slots[req.slot]);
		break;
	default:
		ret = -ENOTTY;
	}
...
static int __init pa_init(void) {
	struct kmem_cache_args vcache_args = {
	...
	pa_vcache = __kmem_cache_create_args("pa_vcache", OBJ_SIZE,&vcache_args,SLAB_HWCACHE_ALIGN | SLAB_TYPESAFE_BY_RCU);
	if (!pa_vcache) 
		return -ENOMEM;
	pa_major = register_chrdev(0, DEVNAME, &pa_fops);
	if (pa_major < 0) {
		kmem_cache_destroy(pa_vcache);
		return pa_major;
	}
...
MODULE_DESCRIPTION("vulnerable UAF lkm for SheafJack v1 Demo");
