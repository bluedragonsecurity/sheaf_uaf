...
static struct kmem_cache *pa_cache;
...
static long pa_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
	case PA_ALLOC: {
		void *obj = kmem_cache_alloc(pa_cache, GFP_KERNEL);
		if (!obj) { 
			ret = -ENOMEM; 
			break; 
		}
	...
	case PA_FREE:
		...
		kmem_cache_free(pa_cache, slots[req.slot]);
		break;
	case PA_PEEK:
		if (!slots[req.slot]) { 
			ret = -ENOENT; 
...
static int __init pa_init(void) {
	...
	pa_cache = __kmem_cache_create_args("pa_victim", OBJ_SIZE, &args, SLAB_HWCACHE_ALIGN | SLAB_TYPESAFE_BY_RCU);
	if (!pa_cache) 
		return -ENOMEM;
	pa_major = register_chrdev(0, DEVNAME, &pa_fops);
	if (pa_major < 0) { 
		kmem_cache_destroy(pa_cache); 
		return pa_major; 
	}
...
MODULE_DESCRIPTION("Vulner UAF lkm for SheafJack v1 Demo");
