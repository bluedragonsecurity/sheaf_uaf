/*
 * vmsg.c - Vulnerable LKM 
 * 2 bugs : race to double free & missing bounds check
 * Tested on linux 7.0-rc1
 * Antonius - bluedragonsec.com
 * Demo for SheafJack v2 via AARW
 */
...
static_assert(sizeof(struct vmsg) == VMSG_OBJ_SIZE, "struct vmsg must be 256 bytes");
...
static int __init vmsg_init(void) {
    ...
    vmsg_cache = kmem_cache_create_usercopy("vmsg",VMSG_OBJ_SIZE, 0, SLAB_HWCACHE_ALIGN | SLAB_TYPESAFE_BY_RCU, 0, VMSG_OBJ_SIZE, NULL);
    if (!vmsg_cache)
        return -ENOMEM;
    ...
    return 0;
err_cdev:  cdev_del(&vmsg_cdev);
err_chr:   unregister_chrdev_region(vmsg_dev, 1);
err_cache: kmem_cache_destroy(vmsg_cache);
...
MODULE_DESCRIPTION("vmsg - demo for SheafJack V2");
