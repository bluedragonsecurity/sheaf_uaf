/*
 * build aarw primitives from pipe_buffer
 * vulner uaf lkm 
 * Antonius - bluedragonsec.com
 * kernel 7.0, KASLR ON, kptr_restrict 2
 */
...
static long pa_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
	case PA_ALLOC: {
		obj = kmalloc(OBJ_SIZE, GFP_KERNEL_ACCOUNT);
		if (!obj) { 
			ret = -ENOMEM; 
			break; 
		}
...
MODULE_DESCRIPTION("UAF vulner lkm for linux 7.0");
