/*
 * lkm for deterministic cross-cache UAF demo using DirtyCred (Linux 7.0)
 * antonius - bluedragonsec.com - linux 7.0
 */
...
#define CCX_OBJ_SIZE     192  /* match sizeof(struct file) HWCACHE-aligned */
struct ccx_req {
	int           idx;
	unsigned long off;
	__u64         val;
...
#define CCX_SHRINK       _IO  (CCX_IOC_MAGIC, 4)  /* drain barn+sheaves+partials -> buddy */
#define CCX_RESTORE      _IOW (CCX_IOC_MAGIC, 5, struct ccx_req)  
#define CCX_RESET        _IO  (CCX_IOC_MAGIC, 6)           
static int                 major;
...
static long ccx_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	if (cmd != CCX_SHRINK && cmd != CCX_RESET && copy_from_user(&r, (void __user *)arg, sizeof(r)))
		return -EFAULT;
	mutex_lock(&ccx_lock);
	switch (cmd) {
	case CCX_ALLOC:      
...
static int __init ccx_init(void) {
	...
	ccx_cache = kmem_cache_create("ccxfile_a", CCX_OBJ_SIZE, 0 , SLAB_HWCACHE_ALIGN | SLAB_NO_MERGE, NULL /* no ctor */);
	if (!ccx_cache) {
...
MODULE_DESCRIPTION("lkm for deterministic cross-cache UAF demo using DirtyCred (Linux 7.0)");
