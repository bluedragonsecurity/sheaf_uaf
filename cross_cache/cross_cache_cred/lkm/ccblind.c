...
#define CC_WRITE        _IOW (CC_IOC_MAGIC, 2, struct cc_req) 
#define CC_READ         _IOWR(CC_IOC_MAGIC, 3, struct cc_req)  
#define CC_FREE         _IOW (CC_IOC_MAGIC, 4, struct cc_req)
#define CC_ALLOC_RUN    _IOW(CC_IOC_MAGIC, 5, struct cc_run)  
#define CC_FREE_RUN     _IOW(CC_IOC_MAGIC, 6, struct cc_run)  

static void cc_ctor(void *p) { 
	memset(p, 0, CC_OBJ_SZ); 
}
...
MODULE_DESCRIPTION("Vulnerable LKM for kernel 7.0");
