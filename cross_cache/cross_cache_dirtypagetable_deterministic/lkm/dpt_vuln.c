/*
 * vuln lkm for uaf exploitation demo with dirtypagetable
...
#define DPT_OBJ_SIZE   128   /* order-0 slab, 32 obj/page, sheaf_cap=60 */
struct dpt_object {
...
struct dpt_dbg {
	int           slot;
	__u64         kaddr; /* kernel virtual addr obj */
	__u64         phys;  /* virt_to_phys(obj)   */
	__u64         pfn;   /* phys >> PAGE_SHIFT  */
	__u64         phys_base;  /* phys_base (VA to PA conversion)  */
	__u64         v2p_in; 
	__u64         v2p_out;
};
...
#define DPT_RESET  _IO  (DPT_IOC_MAGIC, 6)
static int major;
...
static void dpt_ctor(void *p) { 
	memset(p, 0, DPT_OBJ_SIZE); 
}
...
static long do_dbg(struct dpt_dbg *d) {
	...
	if (d->v2p_in == 0xDEADUL) {
		/* DIAGNOSTIC MODE: check if page is still a slab page */
		if (obj) {
...
static long dpt_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
	...
	}
	{
		...
		mutex_lock(&dpt_lock);
		switch (cmd) {
		case DPT_ALLOC: ret = do_alloc(&r); break;
		case DPT_FREE:  ret = do_free(&r);  break;
		case DPT_PEEK:  ret = do_peek(&r);  break;
		case DPT_POKE:  ret = do_poke(&r);  break;
		case DPT_RESET: { int _i; for(_i=0;_i<DPT_MAX_SLOTS;_i++) slots[_i]=NULL; ret=0; break; }
...
static int __init dpt_init(void) {
	...
	dpt_cache = kmem_cache_create("dpt_victim", DPT_OBJ_SIZE, 0,SLAB_HWCACHE_ALIGN | SLAB_ACCOUNT | SLAB_PANIC,dpt_ctor);
...
MODULE_DESCRIPTION("Dirty Pagetable research - linux 7.0");
