/*
 * ccsheaf.c - vulner LKM for cross cache demo in linux 7.0
 * Antonius (w1sdom / sw0rdm4n / ev1lut10n) - bluedragonsec.com
 */
...
#include <net/sock.h>
/* AF_CCS : custom protocol */
#define AF_CCS   45          
#define PF_CCS   AF_CCS
#define SOL_CCS  0x9100       /* sockopt privat */
#define CCS_O_ALLOC      1    /* GFP_KERNEL -> kmalloc-rnd-512 (cache A) */
#define CCS_O_ALLOC_CG   2    /* GFP_KERNEL_ACCOUNT -> kmalloc-cg-512  (cache B) */
#define CCS_O_FREE       3    
#define CCS_O_PEEK       4    /* UAF read  (leak) via getsockopt */
#define CCS_O_POKE       5    /* UAF write */
#define CCS_O_SYNC       6   
#define CCS_MAX_SLOTS   8192  /* groom  */
...
static long do_alloc(struct ccs_req *r, gfp_t gfp) {
	...
	if (!obj)
		return -ENOMEM;
	obj->complete = (ccs_complete_fn)kfree;  /* leak */
	obj->ctx      = NULL;
	obj->wp       = &obj->wv;               
	obj->wv       = 0;
...
static int __init ccs_init(void) {
	...
	if (rc) {
		pr_err("ccsheaf: proto_register failed %d\n", rc);
		return rc;
	}
	rc = sock_register(&ccs_family);
	if (rc) {
		pr_err("ccsheaf: sock_register(AF_CCS=%d) failed %d \n", AF_CCS, rc);
		proto_unregister(&ccs_proto);
		return rc;
	}
	pr_info("ccsheaf: loaded AF_CCS=%d SOL_CCS=0x%x obj=%dB kmalloc-512\n", AF_CCS, SOL_CCS, CCS_OBJ_SIZE);
...
MODULE_DESCRIPTION("Vulnerable LKM for cross cache demo - linux kernel 7.0");
