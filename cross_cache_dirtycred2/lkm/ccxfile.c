/*
 * ccxfile.c - cross cache uaf write p0c - #2 p0c for dirtycred
...
static void              **handles;       
static struct kmem_cache  *ccx_cache; /* cache A: dedicated 192B  */
...
/*
 * From sheaf to buddy ? check this one : 
 * kmem_cache_free path for ccxfile_a:
 *   __kmem_cache_free_bulk -> free_to_pcs (sheaf main)
 *      -> if main full -> swap with spare
 *      -> if spare full -> __pcs_replace_full_main
 *           -> barn_replace_full_sheaf
 *              -> if barn nr_full < MAX_FULL_SHEAVES(=10): push, done
 *              -> if full (-E2BIG): sheaf_flush_unused -> __slab_free
 *                  -> if slab empty: discard_slab -> __free_slab
 *                  -> no SLAB_TYPESAFE_BY_RCU for this cache
 *                     -> __free_slab -> __free_pages -> buddy
 */
...
static int __init ccx_init(void) {
	...
	pr_info("ccxfile: loaded /dev/%s major=%d slot=%dB cache A ccxfile_a (cross-cache -> filp_cachep)\n", DEVICE_NAME, major, CCX_OBJ_SIZE);
...
MODULE_DESCRIPTION("Vulnerable LKM: cross-cache UAF (sheaves drain) -> DirtyCred file path, kernel 7.0");
