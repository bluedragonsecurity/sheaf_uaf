...
static long do_free(struct sheafy_req *r) {
	...
	if (!obj) 
		return -ENOENT;
	kmem_cache_free(sheafy_cache, obj);  /* -> free_to_pcs sheaf */
...
static int __init sheafy_init(void) {
	...
	pr_info("sheafy: loaded major=%d obj_sz=%zu (merge target: cred_jar) /dev/%s\n", major, SHEAFY_OBJ_SZ, DEVICE_NAME);
