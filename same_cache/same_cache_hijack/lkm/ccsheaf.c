/*
 * ccsheaf.c - lkm with uaf for same cache reclaim
...
#define CCS_PEEK   _IOWR('C', 0x03, struct ccs_req)   /* UAF read */
#define CCS_POKE   _IOWR('C', 0x02, struct ccs_req)   /* UAF write */
#define CCS_FIRE   _IOWR('C', 0x07, struct ccs_req)   /* LPE */
...
MODULE_DESCRIPTION("Same cache UAF pOc (uaf write + uaf read for _text leak) for SLUB sheaves kernel 7.0");
